...
await server.start();
console.log(`[express-app] embedded ably-server ready on internal port ${server.port} (pid ${server.child.pid})`);
// Visibility into crash-recovery.
server.on('exit', ({ code, signal }) => console.log(`[express-app] embedded server exited (code=${code} signal=${signal})`));
server.on('restart', (n) => console.log(`[express-app] restarting embedded server (attempt ${n})`));
server.on('ready', (port) => console.log(`[express-app] embedded server (re)ready on internal port ${port} (pid ${server.child?.pid})`));
...
const proxy = mountAblyProxy(app, { server, mountPath: MOUNT });

const httpServer = app.listen(PORT, () => {
  console.log(`[express-app] listening on http://127.0.0.1:${PORT} — Ably under ${MOUNT}, demo at /demo/`);
});
httpServer.on('upgrade', proxy.upgrade); // WebSocket upgrades bypass the Express stack
