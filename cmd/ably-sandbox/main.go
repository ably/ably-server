// Command ably-sandbox is a disposable-instance provisioner for the Ably
...
// single-app. Its working name rides TASK-75's final naming decision.
package main
...
const (
	listenEnv    = "ABLY_SANDBOX_LISTEN"
	serverBinEnv = "ABLY_SANDBOX_SERVER_BIN"
	idleTTLEnv   = "ABLY_SANDBOX_IDLE_TTL"
	logDirEnv    = "ABLY_SANDBOX_LOG_DIR"
	logLevelEnv  = "ABLY_SANDBOX_LOG_LEVEL"
)
...
func run(ctx context.Context, opts runOpts) int {
	fs := flag.NewFlagSet("ably-sandbox", flag.ContinueOnError)
	...
	logLevel := fs.String("log-level", def(opts.Getenv(logLevelEnv), "info"), "log level: debug, info, warn, error (env: "+logLevelEnv+")")
	...
	dir := *logDir
	if dir == "" {
		dir, err = os.MkdirTemp("", "ably-sandbox-*")
		if err != nil {
