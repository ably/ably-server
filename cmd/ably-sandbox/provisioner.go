...
func newProvisioner(cmd serverCommand, logDir string, idleTTL time.Duration, logger *slog.Logger) *provisioner {
	return &provisioner{
		serverCmd:   cmd,
		logDir:      logDir,
		idleTTL:     idleTTL,
...
// is run via `go run ./cmd/ably-sandbox`).
...
func (p *provisioner) startChild(appID, tomlConfig string) (*child, error) {
	...
	// for the provisioner (e.g. ABLY_SERVER_API_KEY, which resolves ahead
