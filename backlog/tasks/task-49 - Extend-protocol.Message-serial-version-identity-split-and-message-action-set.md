...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Message.Serial is the stable message identity; for a create it equals channelSerial:idx and is documented as unchanged across versions
- [ ] #2 Message gains a Version field (MessageVersion: serial, timestamp, clientId, description, metadata) with json+msgpack tags matching Ably's version object
- [ ] #3 MessageAction constants defined: create=0, update=1, delete=2, append=5, pinned to ably-go's MessageAction
- [ ] #4 A create stamps version == serial; the types represent a mutation as serial = target identity with version = the new position
- [ ] #5 A Message carrying action and version round-trips through JSON and msgpack
- [ ] #6 Outbound creates still carry action=0 (TASK-37 behaviour preserved); inbound non-zero actions remain rejected until the mutation path lands
<!-- AC:END -->
