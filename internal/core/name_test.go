...
func TestValidChannelName(t *testing.T) {
	cases := []struct {
		...
		{"::", false},              // leading colon
		{"[meta]log", false},       // a metachannel, which this server does not serve
		{"[?rewind=1]foo", false},  // a channel param, likewise
		{"[filter=abc]foo", false}, // a filtered channel, likewise
