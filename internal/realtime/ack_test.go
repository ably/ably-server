...
// TestAckCountPerProtocolMessage reproduces the pending-emitter accounting
// sequence some SDKs (e.g. ably-go) use: a create then an update over ONE
// connection. Some SDKs (e.g. ably-go) ack queue[:count] FRAMES per ACK,
// panicking when count exceeds the pending frames. So every inbound protocol message must
...
func TestAckCountPerProtocolMessage(t *testing.T) {
	...
	for i, want := range []int64{1, 2} {
		...
		if a.Count != 1 {
			t.Errorf("frame %d (msgSerial %d): Count = %d, want 1", i+1, a.GetMsgSerial(), a.Count)
		}
		if a.GetMsgSerial() != want {
			t.Errorf("ACK %d: msgSerial = %d, want %d", i+1, a.GetMsgSerial(), want)
		}
	}
}
