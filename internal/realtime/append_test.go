...
func TestAppendIncompatibleDataNacked(t *testing.T) {
	...
	// Append a binary value onto a string message: concatenation is
	// defined only string-onto-string / binary-onto-binary. (A number would
	// normalise to a string per the reference ingress rules, and
	// so would concatenate; binary-onto-string is a genuine type mismatch.)
	sendMutation(t, pub, "room", 2, &protocol.Message{Action: protocol.MessageAppend, Serial: target, Data: []byte{0, 1, 2}})
	for {
		f := readFrame(t, pub, protocol.FormatJSON, 2*time.Second)
		if f.Action == protocol.ActionMessage {
			continue // skip the create echo
		}
		if f.Action != protocol.ActionNack || f.GetMsgSerial() != 2 {
...
// to a subscriber that routes/filters by name, which is the durable
