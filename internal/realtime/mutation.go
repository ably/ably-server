...
func (c *connection) handleMutation(ctx context.Context, msg *protocol.ProtocolMessage) {
	...
	if len(msg.Messages) != 1 {
		c.logger.Warn("mutation must carry exactly one message; rejecting",
			"channel", msg.GetChannel(), "count", len(msg.Messages), "msgSerial", msgSerial)
	...
	if !m.Action.IsMutation() || m.Serial == "" {
		c.logger.Warn("mutation missing action or target serial; rejecting",
			"channel", msg.GetChannel(), "action", m.Action.String(), "msgSerial", msgSerial)
		c.enqueueNack(ctx, msgSerial, &protocol.ErrorInfo{
	...
	// A mutation may carry an operation clientId in version.clientId (the
	// SDK's operation object); validate it against the connection identity
	// exactly like a publish clientId, then stamp it as the operator
	// (MergeVersion reads it back from Message.ClientID). The message's
	// creator clientId is carried forward from the target by the merge.
	opClientID := ""
	if m.Version != nil {
		opClientID = m.Version.ClientID
	}
	stamped, ok := auth.MessageClientID(c.clientID, opClientID)
	if !ok {
		c.logger.Warn("mutation operation clientId not permitted; rejecting",
			"channel", msg.GetChannel(), "msgSerial", msgSerial)
		c.enqueueNack(ctx, msgSerial, &protocol.ErrorInfo{
			Message:    "operation clientId not permitted",
			Code:       40012,
			StatusCode: 400,
		})
		return
