...
// ConnectionDetails with the resolved clientId, a connectionKey equal to
// the connectionId, the advertised limits, and maxIdleInterval aligned to
// the server heartbeat cadence (DESIGN.md §2.1, §8).
func TestConnectedCarriesConnectionDetails(t *testing.T) {
	...
	if cd.ConnectionKey != connected.ConnectionID {
		t.Errorf("connectionKey = %q, want it to equal connectionId %q", cd.ConnectionKey, connected.ConnectionID)
	}
