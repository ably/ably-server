...
// over Basic (?key=) auth: the connection resolves to the key's
...
func TestAttachModesFromRestrictedKey(t *testing.T) {
	...
	rt := NewServer([]auth.APIKey{key}, manager, time.Hour, logging.New(slog.DiscardHandler), nil, nil)
