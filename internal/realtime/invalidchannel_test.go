...
// stays usable afterwards (TASK-102). A long heartbeat keeps an idle
...
func TestAttachInvalidChannelNameErrors(t *testing.T) {
	...
	if f.Channel != ":hell" {
		t.Fatalf("ERROR channel = %q, want %q", f.Channel, ":hell")
	}
...
// channel name is NACKed with 40010 (TASK-102).
