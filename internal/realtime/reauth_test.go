...
// A live reauth that narrows capability below what an attached channel
// needs fails that channel — a channel-scoped ERROR (which drives the
// SDK's channel to FAILED), not a plain DETACHED — while a channel that
// keeps at least one permitted mode is left attached, untouched (RTC8a1).
func TestReauthCapabilityDowngradeFailsChannel(t *testing.T) {
	srv, _ := newTestServer(t, time.Hour)
	ws := dialWithToken(t, srv, signToken(t, `{"*":["*"]}`, "", time.Hour))
	if f := readFrame(t, ws, protocol.FormatJSON, 2*time.Second); f.Action != protocol.ActionConnected {
		t.Fatalf("first frame = %v, want CONNECTED", f.Action)
	}

	sendFrame(t, ws, protocol.FormatJSON, &protocol.ProtocolMessage{Action: protocol.ActionAttach, Channel: new("notfoo")})
	if f := readFrame(t, ws, protocol.FormatJSON, 2*time.Second); f.Action != protocol.ActionAttached || f.GetChannel() != "notfoo" {
		t.Fatalf("attach notfoo = %+v, want ATTACHED", f)
	}
	sendFrame(t, ws, protocol.FormatJSON, &protocol.ProtocolMessage{Action: protocol.ActionAttach, Channel: new("foo")})
	if f := readFrame(t, ws, protocol.FormatJSON, 2*time.Second); f.Action != protocol.ActionAttached || f.GetChannel() != "foo" {
		t.Fatalf("attach foo = %+v, want ATTACHED", f)
	}

	// Reauth with a capability granting nothing on "notfoo" but still
	// permitting (narrower than the default modes, but nonzero) "foo".
	narrow := signToken(t, `{"foo":["publish"]}`, "", time.Hour)
	sendFrame(t, ws, protocol.FormatJSON, &protocol.ProtocolMessage{
		Action: protocol.ActionAuth,
		Auth:   &protocol.AuthDetails{AccessToken: narrow},
	})

	f := readFrame(t, ws, protocol.FormatJSON, 2*time.Second)
	if f.Action != protocol.ActionError || f.GetChannel() != "notfoo" || f.Error == nil || f.Error.Code != 40160 {
		t.Fatalf("frame = %+v, want ERROR 40160 for notfoo", f)
	}

	// The connection-level reauth still completes; "foo" gets no frame at
	// all (it wasn't touched) — the next frame is the reauth's CONNECTED.
	f = readFrame(t, ws, protocol.FormatJSON, 2*time.Second)
	if f.Action != protocol.ActionConnected {
