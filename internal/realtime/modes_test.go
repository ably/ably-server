...
// including unrecognised-token tolerance (TASK-103).
...
// falls back to the flags (TASK-103).
...
// ATTACHED.params.modes (TASK-103).
...
// is attached to without the publish mode is NACKed 40160 (TASK-103); the
// symmetric enforcement to presence.go, so ably-js checkCantPublish sees the
