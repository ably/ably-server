...
func (c *connection) handleAnnotation(ctx context.Context, msg *protocol.ProtocolMessage) {
	...
	if msg.GetChannel() == "" || len(msg.Annotations) == 0 {
	...
	if !ok || !a.hasMode(protocol.FlagAnnotationPublish) || !c.capability().Permits(msg.GetChannel(), auth.OpAnnotationPublish) {
	...
	for _, an := range msg.Annotations {
		...
		if !ok {
			c.logger.Warn("ANNOTATION clientId rejected", "channel", msg.GetChannel(),
		...
		if verr := an.Validate(); verr != nil {
			c.logger.Warn("ANNOTATION validation failed; rejecting",
				"channel", msg.GetChannel(), "msgSerial", msgSerial, "reason", verr.Message)
