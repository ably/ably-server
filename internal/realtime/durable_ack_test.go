...
// the Store path, exercising the off-loop publish worker (TASK-20).
...
// promptly — and the ACK is emitted only once the store commits (TASK-20).
func TestPublishAckAfterCommitNonBlocking(t *testing.T) {
	...
	if f := readFrame(t, ws, protocol.FormatJSON, 2*time.Second); f.Action != protocol.ActionAttached || f.Channel != "other" {
		t.Fatalf("read loop blocked on in-flight store: got %v/%q, want ATTACHED/other", f.Action, f.Channel)
	}
...
// rather than optimistically ACKing (TASK-20).
...
// preserves per-connection ACK ordering (TASK-20).
