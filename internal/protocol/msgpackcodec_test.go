...
// TestMsgpackEncoderFieldParity is the drift guard: the custom
...
// TestEmptyDataSurvivesRoundTrip pins empty-Data survival: an empty-string (and
	...
	t.Run("Message", func(t *testing.T) {
		for _, data := range []any{"", []byte{}} {
			...
			for _, f := range []Format{FormatJSON, FormatMsgpack} {
				wire, err := Marshal(&ProtocolMessage{Action: ActionMessage, Channel: new("c"), Messages: []*Message{m}}, f)
				if err != nil {
